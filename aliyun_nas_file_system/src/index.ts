...
import { NasFileSystem } from './impl/nas_filesystem';
import { GLogger } from 'base_aliyun_resource';

export default class ComponentNasFileSystem {
  ...
  public async exec(inputs: IInputs, command: string) {
    ...
    const nasFileSystem = new NasFileSystem(inputs);
    return await nasFileSystem.deploy();
  }
}
